...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    //Using getElementsByClassName as a faster method for selecting pizzas instead of querySelectorAll
    var pizzaRand = document.getElementsByClassName("randomPizzaContainer");
    //Pulled varibles out of for loop that made sense
    var dx = determineDx(document.getElementsByClassName("randomPizzaContainer")[0], size);
    var newwidth = (document.getElementsByClassName("randomPizzaContainer")[0].offsetWidth + dx) + 'px';

    //Taking array length variable out of the for loop so it doesnt' constantly calculate.
    var pizzaLen = pizzaRand.length;
    for (var i=0; i < pizzaLen; i++){
      pizzaRand[i].style.width = newwidth;
    }
  }
...
// Similar to the other loops, moved variables out that didn't make sense so they didn't get processed over and over
...
function updatePositions() {
  ...
  //Changed to the more optimal getElementsByClassName
  var items = document.getElementsByClassName("mover");
  //Moved document scroll out from the loop
  var phaseScroll = document.body.scrollTop / 1250;
  //Moved array length from for loop
  var itemsLen = items.length;

  for (var i = 0; i < itemsLen; i++) {
    var phase = Math.sin(phaseScroll + (i % 5));
    translateX = (i % 8) * 256 + 100 * phase + "px";
    //Pushed everything into translate3d because it pushes the animation into hardware acceleration and let's the GPU take over.
    items[i].style.transform = "translate3d("+translateX+", 0, 0)";
  }
